...
// Fail fast rather than silently falling back to a baked-in project — a
// hardcoded fallback here would mean the real Supabase URL/key ships inside
// the built JS bundle forever, regardless of what's in .env locally.
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;

if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.error(
    "[Supabase] Missing VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY. Copy .env.example to .env and fill in your project's values."
  );
}
...
// Logging helper — success logs are dev-only noise (this fires on every
// polling tick and every realtime event); errors always surface since they
// matter in production too.
...
// This is the hot path: it's called on a poll timer, on every realtime event,
// and on every tab focus. Historically it had no filter at all, so it pulled
// EVERY order the restaurant had ever taken (plus their items and menu-item
// joins) on every single one of those triggers, and kept getting slower as
// more history accumulated. The live floor (table map / KDS / billing) only
// ever needs orders that are still open, so we filter to those server-side.
// Full history for Analytics goes through fetchOrderHistory below instead.
const LIVE_ORDER_STATUSES = ["New", "Preparing", "Ready", "Served", "Payment Pending"];

export const fetchOrders = async () => {
  try {
    // 1. Try join query first — filtered to orders still relevant to the
    // live floor instead of the entire orders table.
    const { data: ordersData, error: ordersErr } = await supabase
      .from("orders")
      .select("*, order_items(*, menu_items(*)), restaurant_tables(*)")
      .or(`order_status.in.(${LIVE_ORDER_STATUSES.join(",")}),payment_status.eq.Pending`)
    ...
    // Still bounded — no unfiltered full-table scans.
    ...
    const { data: rawItems } = orderIds.length
...
export const createOrderInDb = async ({
  ...
  try {
    ...
    // NOTE: this used to also attempt an RPC call to a "create_complete_order"
    // Postgres function before falling back to this direct insert. That RPC
    // function is never defined anywhere in this project's schema (see
    // supabase_schema.sql) — calling it always fails, which is fine on its
    // own since the code fell through to this REST path either way. The real
    // danger was the *success* branch: if that RPC ever did exist (e.g. a
    // leftover from a different/previous database) but returned its result
    // in a shape this code didn't expect (an array instead of a plain object,
    // a differently-named field, etc.), `rpcRes?.order_id` would read as
    // undefined even though the RPC had already inserted a real row —
    // and execution would fall through to ALSO run the insert below,
    // creating two orders from a single call. Removed entirely so exactly
    // one code path creates exactly one order, with no ambiguity.
    const orderPayload = {
