...
// Hoisted to module scope: these are fully static (no dependency on props,
// state, or context), so defining them here means they're created exactly
// once, ever, instead of as a fresh object literal on every render of
// RevenueAnalytics. React-chartjs-2 does a reference-equality-driven redraw
// on `data`/`options` changes, so recreating these every render (they used
// to live inside the component) was forcing full canvas redraws every time
// unrelated local state changed — e.g. every keystroke in the search box.
const HOURLY_BAR_DATA = {
...
export const RevenueAnalytics = ({ viewMode = "analytics" }) => {
  ...
  // This merge (including a localStorage read + JSON.parse) previously ran
  // on every render of this component — including every keystroke in the
  // order-history search box and every payment-method filter click, neither
  // of which change the underlying order set. useMemo scopes the actual
  // merge to only when its real inputs (paidTransactions, dbOrders) change;
  // the search/filter step below stays a separate, cheap memo on top of it.
  const allCompletedOrders = useMemo(() => {
    ...
    const merged = [...(paidTransactions || []), ...localPaid];

    dbOrders.forEach((dbo) => {
      if (!merged.some((o) => (o.id && String(o.id) === String(dbo.id)) || (o.preorder_ticket && o.preorder_ticket === dbo.preorder_ticket))) {
        merged.push({
          id: dbo.id,
          orderId: dbo.id,
    ...
    return merged;
  }, [paidTransactions, dbOrders]);

  const totalCalculatedRevenue = useMemo(
    () => allCompletedOrders.reduce((sum, ord) => sum + Number(ord.amount || ord.total || 0), 0) || 31659,
    [allCompletedOrders]
  );
  ...
  const filteredHistoryOrders = useMemo(
    ...
    [allCompletedOrders, searchQuery, paymentFilter]
  );

  // Chart configs — see hoisted static data below the component (they were
  // plain static literals redefined here every render, which made
  // react-chartjs-2 treat the chart data as "changed" and redraw the canvas
  // on every keystroke in the unrelated search box above).
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: "20px", marginBottom: "28px" }}>
            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "16px", border: "1px solid #E5E2D9" }}>
              ...
              <div style={{ height: "240px" }}><Bar data={HOURLY_BAR_DATA} options={HOURLY_BAR_OPTIONS} /></div>
            </div>

            <div style={{ background: "#ffffff", padding: "20px", borderRadius: "16px", border: "1px solid #E5E2D9" }}>
              <h3 style={{ margin: "0 0 16px", fontSize: "15px", fontWeight: 800, color: "#1F1F1F" }}>Top Selling Items Breakdown</h3>
              <div style={{ height: "240px" }}><Doughnut data={TOP_ITEMS_DATA} options={TOP_ITEMS_OPTIONS} /></div>
            </div>
          </div>
